...
public class UseItem {
	...
	public static void ItemonObject(Client c, int objectID, int objectX, int objectY, int itemId) {
		if (!c.getItems().playerHasItem(itemId, 1))
			return;
		switch(objectID) {
			case 2638:  //if Fountain of heroes
			if(itemId == 1704){//if ammy of glory with 0 charges
			c.getItems().deleteItem(1704, c.getItems().getItemSlot(1704), 1);
			c.getItems().addItem(1712,1);
			}
			else
			{
			c.sendMessage("The fountain doesn't respond to that item.");
			}
			break;
			case 2452:  //if air altar stone
			if(itemId == 1438)  //if air tally
			{
			c.teleportToX = 2845;
			c.teleportToY = 4832;
			c.heightLevel = 0;
			...
			{
			c.sendMessage("The ruins doesn't respond to that item.");
			}
			break;
			case 2454:  //if water altar stone
			if(itemId == 1444)  //if water tally
			{
			c.teleportToX = 2713;
			c.teleportToY = 4836;
			c.heightLevel = 0;
			...
			break;
			case 2456:  //if fire altar stone
			if(itemId == 1442)  //if fire tally
			{
			c.teleportToX = 2584;
			c.teleportToY = 4836;
			c.heightLevel = 0;
			...
			break;
			case 2455:  //if earth altar stone
			if(itemId == 1440)  //if earth tally
			{
			c.teleportToX = 2660;
			c.teleportToY = 4839;
			c.heightLevel = 0;
